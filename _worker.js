  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      ...
      if (cacheable) {
        ...
        if (cached) {
          ...
          return withSecurityHeaders(maybeCompress(request, withStaticCacheHeaders(withMetricHeaders(cached, { 'x-edge-cache': 'HIT' }), url.pathname)));
        }
    ...
    try {
      if (env.ASSETS) {
        ...
        return withSecurityHeaders(maybeCompress(request, withStaticCacheHeaders(staticRes, url.pathname)));
      }
    } catch (e) {
