...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case "book_appointment": return jsonRes(bookAppointment(data, cfg));
      case "subscribe_newsletter": return jsonRes(subscribeNewsletter(data, cfg));
      case "confirm_newsletter": return jsonRes(confirmNewsletter(data, cfg));
      case "get_clinic_content": return jsonRes(getClinicContent(data, cfg));
...
function normalizeSafeInput_(value, maxLength) {
  var normalized = normalizePlainText_(value);
  var limit = Number(maxLength || 255);
  if (containsHtmlMarkup_(String(value || ""))) {
    throw new Error("Input tidak valid: tag HTML tidak diizinkan.");
  }
...
function normalizePhoneInput_(value) {
  var raw = String(value || "").replace(/[^\d+]/g, "").trim();
  var digits = "";
  if (!raw) return "";
  if (raw.charAt(0) === "+") {
    return "+" + raw.substring(1).replace(/\D/g, "");
  }
  digits = raw.replace(/\D/g, "");
  if (!digits) return "";
  if (digits.indexOf("62") === 0) return "+" + digits;
  if (digits.charAt(0) === "0") return "+62" + digits.substring(1);
  return "+" + digits;
}
...
function ensureSheetWithHeaders_(sheetName, headers) {
  ...
  var i = 0;
  if (!name) throw new Error("Nama sheet tidak valid.");
  sheet = ss.getSheetByName(name);
  if (!sheet) {
    sheet = ss.insertSheet(name);
  }
  normalizedHeaders = Array.isArray(headers) ? headers : [];
  if (normalizedHeaders.length > 0) {
    firstRow = sheet.getRange(1, 1, 1, normalizedHeaders.length).getValues()[0];
    headerMismatch = false;
    for (i = 0; i < normalizedHeaders.length; i++) {
      if (String(firstRow[i] || "").trim() !== String(normalizedHeaders[i] || "").trim()) {
        headerMismatch = true;
        break;
      }
    }
...
function generateSeoSlug_(value) {
  return String(value || "")
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, " ")
    .trim()
...
function validateCaptchaMath_(answer, expected) {
  var given = Number(String(answer || "").trim());
  var target = Number(String(expected || "").trim());
  if (!isFinite(given) || !isFinite(target)) return false;
  return given === target;
}

function throttleSimple_(scopeKey, ttlSeconds) {
  ...
  cache.put(key, "1", Math.max(10, Number(ttlSeconds || 30)));
  return false;
}

function bookAppointment(data, cfg) {
  var fullName = "";
  var email = "";
  var phone = "";
  var treatment = "";
  ...
  var sourceUrl = "";
  var throttleKey = "";
  var appointmentSheet = null;
  var contactEmail = "";
  cfg = cfg || getSettingsMap_();
  fullName = normalizeSafeInput_(data.full_name, 100);
  email = normalizeSafeInput_(data.email, 120).toLowerCase();
  phone = normalizePhoneInput_(data.phone);
  treatment = normalizeSafeInput_(data.treatment, 80);
  preferredDate = normalizeSafeInput_(data.preferred_date, 30);
  notes = normalizeSafeInput_(data.notes, 800);
  sourceUrl = normalizeSafeInput_(data.source_url, 255);

  if (!fullName || !email || !phone || !treatment || !preferredDate) {
    return { status: "error", message: "Lengkapi data appointment terlebih dahulu." };
  }
  if (!isValidEmail_(email)) {
    return { status: "error", message: "Format email tidak valid." };
  }
  if (!isValidPhone_(phone)) {
    return { status: "error", message: "Format nomor HP tidak valid." };
  }
  if (!validateCaptchaMath_(data.captcha_answer, data.captcha_expected)) {
    return { status: "error", message: "Captcha tidak sesuai." };
  }

  throttleKey = "appt:" + Utilities.base64EncodeWebSafe(email + ":" + phone).substring(0, 40);
  if (throttleSimple_(throttleKey, 30)) {
    return { status: "error", message: "Terlalu banyak request. Coba lagi beberapa saat." };
  }

  appointmentSheet = ensureSheetWithHeaders_("Appointment_Requests", [
    "created_at", "full_name", "email", "phone", "treatment", "preferred_date", "notes", "source_url", "status"
  ]);
  ...
  contactEmail = String(getCfgFrom_(cfg, "contact_email") || "").trim();
  if (contactEmail && isValidEmail_(contactEmail)) {
    sendEmail(
      contactEmail,
      "Appointment Baru - Zhost Klinik",
      [
        "Permintaan appointment baru diterima:",
        "Nama: " + fullName,
        "Email: " + email,
        "No HP: " + phone,
        "Treatment: " + treatment,
  ...
  return {
    status: "success",
    message: "Appointment berhasil dikirim. Tim kami akan menghubungi Anda segera."
  };
}

function subscribeNewsletter(data, cfg) {
  var email = "";
  var sourceUrl = "";
  var throttleKey = "";
  var sheet = null;
  ...
  if (!email || !isValidEmail_(email)) {
    return { status: "error", message: "Email newsletter tidak valid." };
  }

  throttleKey = "newsletter:" + Utilities.base64EncodeWebSafe(email).substring(0, 40);
  if (throttleSimple_(throttleKey, 30)) {
    return { status: "error", message: "Terlalu banyak request. Coba lagi nanti." };
  }

  sheet = ensureSheetWithHeaders_("Newsletter_Subscribers", [
    "created_at", "email", "status", "token", "token_expiry", "confirmed_at", "source_url"
  ]);
  ...
  expiresIso = Utilities.formatDate(expiresAt, Session.getScriptTimeZone(), "yyyy-MM-dd'T'HH:mm:ssXXX");
  createdAt = nowIsoTimestamp_();
  ...
  if (existingRow > 0) {
    ...
    sheet.appendRow([createdAt, email, "pending", token, expiresIso, "", sourceUrl]);
  }

  baseUrl = String(getCfgFrom_(cfg, "public_base_url") || "https://klinik.zhost.digital").replace(/\/+$/, "");
  confirmUrl = baseUrl + "/newsletter-confirm.html?token=" + encodeURIComponent(token);
  sendEmail(
    email,
    "Konfirmasi Newsletter Zhost Klinik",
    [
      "Terima kasih telah berlangganan newsletter Zhost Klinik.",
      "Klik link berikut untuk konfirmasi (double opt-in):",
      confirmUrl,
      "Link berlaku hingga: " + expiresIso
    ].join("\n")
  );

  return {
    status: "success",
    message: "Cek email Anda untuk konfirmasi langganan newsletter."
  };
}

function confirmNewsletter(data, cfg) {
  var token = "";
  var sheet = null;
  var rows = [];
  var rowIndex = -1;
  ...
  var expiryDate = null;
  cfg = cfg || getSettingsMap_();
  token = normalizeSafeInput_(data.token, 128).toLowerCase();
  if (!/^[a-f0-9]{32,128}$/.test(token)) {
    return { status: "error", message: "Token konfirmasi tidak valid." };
  }
  ...
  rowData = null;
  for (i = 1; i < rows.length; i++) {
    if (String(rows[i][3] || "").trim().toLowerCase() === token) {
      rowIndex = i + 1;
      rowData = rows[i];
      break;
    }
  ...
  if (rowIndex < 0 || !rowData) {
    return { status: "error", message: "Token newsletter tidak ditemukan." };
  }

  status = String(rowData[2] || "").trim().toLowerCase();
  if (status === "active") {
    return { status: "success", message: "Newsletter sudah aktif sebelumnya." };
  }

  expiryRaw = String(rowData[4] || "").trim();
  expiryDate = expiryRaw ? new Date(expiryRaw) : null;
  if (!expiryDate || isNaN(expiryDate.getTime()) || expiryDate.getTime() < Date.now()) {
    return { status: "error", message: "Token konfirmasi sudah kedaluwarsa." };
  }
  ...
  try { bumpPublicCacheState_(["pages"]); } catch (e) {}
  return { status: "success", message: "Newsletter berhasil dikonfirmasi." };
}

function getClinicContent(data, cfg) {
  var requestedType = "";
  var sheet = null;
  var rows = [];
  var items = [];
  ...
  var summary = "";
  cfg = cfg || getSettingsMap_();
  requestedType = normalizeSafeInput_(data.content_type, 32).toLowerCase();
  sheet = ensureSheetWithHeaders_("Clinic_Content", [
    "content_type", "title", "slug", "summary", "payload_json", "updated_at"
  ]);
  rows = sheet.getDataRange().getValues();
  items = [];

  for (i = 1; i < rows.length; i++) {
    rowType = String(rows[i][0] || "").trim().toLowerCase();
    if (!rowType) continue;
    if (requestedType && rowType !== requestedType) continue;

    payload = {};
    payloadRaw = String(rows[i][4] || "").trim();
    if (payloadRaw) {
      try { payload = JSON.parse(payloadRaw); } catch (e) { payload = {}; }
    }

    title = normalizeSafeInput_(rows[i][1], 140);
    slug = generateSeoSlug_(rows[i][2] || title);
    summary = normalizeSafeInput_(rows[i][3], 300);

    items.push({
      content_type: rowType,
      title: title,
      slug: slug,
      summary: summary,
      payload: payload,
      updated_at: String(rows[i][5] || "")
    });
  }
